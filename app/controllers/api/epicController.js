...
let epicService = require('../../services/epicService');
...
router.get(
    ...
    asyncHandler(async (req, res) => {
        ...
        const result = await epicService.getAll(req, page, pageSize);

        if (result.length === 0) {
            res.send({success:false, message:res.__('api.client.get.empty')});
        } else {
            let total = await epicService.getAllCount();
...
router.get(
    '/dept/epic',
     authJwt(),
    asyncHandler(async (req, res) => {
        const result = await epicService.getEpicByDepts(req.query.dept);
...
router.get(
    ...
    authJwt(),
    asyncHandler(async (req, res) => {
        const result = await epicService.getTaskByEpic(req.query.epicId);
...
router.post(
    ...
    asyncHandler(async (req, res) => {
        ...
        if (errors) {
            ...
            const id = req.body.id;
            console.log("body " + id);
            const result = await epicService.saveCustom(req, id);
