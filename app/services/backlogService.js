...
class BacklogService {
    ...
    static async getAll(req, page, pageSize) {
        ...
        const result = await db.simpleExecute("select ab.dept_id ,ab.epic_desc, ab.user_story_id,ab.user_story_task ,ab.epic_status ,ab.workorder_ref,ab.seq_work_id,ab.seq_backlog_id from  (select rownum as rn ,dept_id,epic_desc, user_story_id,user_story_task,epic_status ,workorder_ref,seq_work_id,seq_backlog_id from SCI_BACKLOG_MASTER  where epic_status =:search0 and  dept_id like :search1 and epic_desc like :search2 and seq_work_id like :search3  and workorder_ref like :search4) ab where ab.rn between :startlimit and :endlimit ",[searchByStatus,searchByDeptParam,searchByDescParam,searchByWorkParam,searchByWorkDescParam,newPage,parseInt(newPage) +parseInt(pageSize)]
        );
        result.rows.forEach((row) => {
            resultArray.push(new BackLogValueObject(row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7]));
        });
