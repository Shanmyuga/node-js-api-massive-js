...
class epicService {
    ...
    static async getWorkOrders(req) {
        ...
        const result = await db.simpleExecute("select job_Desc ,seq_work_id from SCIGENICS.sci_workorder_master where word_order_Type in ('Fermenter','Service') and wo_Status='Y'"
        );
        let droparray = new Array();
    ...
    static async save(req, id) {
      ...
      let standardEpicId = req.body.standard_epic_id;
        ...
        standardEpicId.forEach(row => {
            ...
            db.simpleExecute("    INSERT INTO \"SCIGENICS\".\"SCI_BACKLOG_MASTER\" (          seq_backlog_id,          dept_id,          user_story_task,          epic_desc,          user_story_id,          created_date,          created_by,          epic_status,          updated_by,          updated_date,          workorder_ref     ,seq_work_id,stage_desc,stage_id ) VALUES (          sci_backlog_master_seq.NEXTVAL,          :dept_id,          (select epic_desc||'-'|| stage_desc|| '-' || user_story_task  from SCI_STANDARD_EPIC_DATA where user_Story_id = :user_Story_id),        (select epic_desc from SCI_STANDARD_EPIC_DATA where user_Story_id = :user_Story_id),          :user_story_id,          sysdate,         :logged_in_user,         'BKLOG',           :logged_in_user,        sysdate,          :work_order_ref,          (select seq_work_id from SCI_WORKORDER_MASTER where job_desc = :work_order_ref) ,  (select stage_desc from SCI_STANDARD_EPIC_DATA where user_Story_id = :user_Story_id) ,(select stage_id from SCI_STANDARD_EPIC_DATA where user_Story_id = :user_Story_id)  )",
...
for(let entry of arrayinternalId) {
    ...
    static async saveAll(req, id) {
        ...
        db.simpleExecute(" insert into SCI_BACKLOG_MASTER (SEQ_BACKLOG_ID, DEPT_ID, USER_STORY_TASK, EPIC_DESC, USER_STORY_ID, CREATED_DATE, CREATED_BY, EPIC_STATUS, UPDATED_BY, UPDATED_DATE, WORKORDER_REF, SEQ_WORK_ID,SUMMARY_ID,MILESTONE,stage_id,stage_desc,seq_st_epic_id)      (select SCI_BACKLOG_MASTER_SEQ.nextval ,    DEPT_ID,  nvl(USER_STORY_TASK,'NA'),  EPIC_DESC,  USER_STORY_ID,      sysdate,  :CREATED_BY,  'INTERNAL_BKLOG',  :updated_by,  sysdate,:job_Dec,  (select seq_work_id from SCI_WORKORDER_MASTER where job_Desc = :jobdesc) ,summary_id ,milestone ,stage_id ,stage_Desc,seq_st_epic_id  from SCI_STANDARD_EPIC_DATA st where USER_STORY_ID not in (select USER_STORY_ID from SCI_BACKLOG_MASTER bm where bm.workorder_ref = :workorder_ref)) ",
